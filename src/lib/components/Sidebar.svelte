...
<nav class="w-[46px] flex-shrink-0 flex flex-col items-center py-2.5 pb-5 gap-1 bg-base-200 shadow-[2px_0_12px_rgba(0,0,0,0.4)] z-10 overflow-y-auto">
    <button class="nav-btn" class:active={activeView.kind === "query"} title="Search" onclick={onactivatesearch}>
        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
            <path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/>
        </svg>
    </button>
    <button class="nav-btn" class:active={activeView.kind === "history"} title="History" onclick={onloadhistory}>
        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
    ...
    <button class="nav-btn" class:active={activeView.kind === "queue"} title="Queue" onclick={onactivatequeue}>
        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
    ...
    {#each categories as cat}
        <button class="nav-btn" class:active={isactive(cat)} title={cat.label} onclick={() => onloadsearch(cat.sorting)}>
            <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                <path d={cat.icon}/>
            </svg>
        </button>
    ...
    <button class="nav-btn" title="Undo" onclick={onundowallpaper} disabled={undoing}>
        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
